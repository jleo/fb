m = function () {
    ...
    emit({year:year,month:month,day:day}, {net: this.delta, bet: this.bet, count: 1,correct: this.delta > 0?1:0, push:this.delta==0?1:0,lose:this.delta<0?1:0,total:1});
}
r = function (k, vals) {
    var totalBet = 0
    var net = 0;
...
res = db.transactionDate.mapReduce(m, r, {finalize: f, out: {replace:"summaryDateByDateOnly"}});